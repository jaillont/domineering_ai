...
@jit(nopython=True)
def UCT(move,mean_score,parent_n,n):
    coef=0.1
...
def MCTS(B,n_turn,n_games,game_tree,games_trees,games_scores):
    ...
    #print("gametree",n_turn)
    #game_nodes=np.zeros(0)
    if n_turn <= 200 or games_trees == []:
        ...
        for move in range(B[-1]):
            ...
            for i in range(games_trees.shape[0]):
                ...
                if current_node_count != 0:
                    value=UCT(move = B[move],mean_score=node_scores,parent_n=parent_node_count,n=current_node_count)
                    print(value)
    ...
    for i in range(n_games):
        ...
        # Si c'est au tour du joueur 0
            ...
            # On lance l'ia 0 pour qu'elle choisisse son coup
                ...
                id_move = MCTS(B,n_turn,n_games,game_tree,non_zero_games_trees,non_zero_games_scores) 
                #print(id_move)
